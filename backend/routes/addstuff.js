...
const multer = require('multer');
const path = require('path');
const fs = require('fs').promises; // fs 모듈 (파일 시스템)
const dotenv = require('dotenv');
dotenv.config();
...
// ingredientService 모듈 가져오기 (사용자 식재료 저장 용도)
...
// 2. 파일 경로 및 디렉토리 관리
const DATA_DIR = path.join(__dirname, '..', 'data');
const RECIPES_MASTER_DATA_FILE = path.join(DATA_DIR, 'recipes.js'); // recipes.js 파일 경로
const TEMP_MASTER_DATA_FILE = path.join(DATA_DIR, 'temp_ingredient_master_data.json'); // temp_ingredient_master_data.json 파일 경로

// 전역 변수로 마스터 데이터 캐시
let initialRecipesMasterData = []; // recipes.js에서 로드될 데이터
let tempIngredientMasterData = []; // temp_ingredient_master_data.json에서 로드될 데이터


// 데이터 디렉토리가 없으면 생성하는 함수
async function ensureDataDirectoryExists() {
    try {
        await fs.mkdir(DATA_DIR, { recursive: true });
        console.log(`데이터 디렉토리 확인 또는 생성 완료: ${DATA_DIR}`);
    } catch (error) {
        console.error('데이터 디렉토리 생성 실패:', error);
    }
}

// recipes.js 파일에서 데이터 읽기 (서버 시작 시 한 번 로드)
async function readRecipesMasterData() {
    try {
        // recipes.js는 Node.js 모듈이므로 require()를 사용하여 로드합니다.
        // require는 캐싱되므로 한 번만 로드됩니다.
        const loadedData = require(RECIPES_MASTER_DATA_FILE);
        console.log('recipes.js 파일 로드 완료.');
        // require() 결과가 배열인지 확인
        return Array.isArray(loadedData) ? loadedData : [];
    } catch (error) {
        if (error.code === 'MODULE_NOT_FOUND') { // 파일이 없는 경우
            console.warn('recipes.js 파일이 없습니다. 빈 마스터 데이터로 시작합니다.');
            return [];
        }
        console.error('recipes.js 로드 실패:', error);
        return [];
    }
}

// temp_ingredient_master_data.json 파일에서 데이터 읽기
async function readTempMasterData() {
    try {
        const data = await fs.readFile(TEMP_MASTER_DATA_FILE, 'utf8');
        return JSON.parse(data);
    } catch (error) {
        if (error.code === 'ENOENT') { // 파일이 없으면 빈 배열 반환
            console.log('temp_ingredient_master_data.json 파일이 없어 새로 생성합니다.');
            return [];
        }
        console.error('temp_ingredient_master_data.json 읽기 실패:', error);
        return [];
    }
}

// temp_ingredient_master_data.json 파일에 데이터 쓰기
async function writeTempMasterData(data) {
    try {
        await fs.writeFile(TEMP_MASTER_DATA_FILE, JSON.stringify(data, null, 4), 'utf8');
        console.log('temp_ingredient_master_data.json 업데이트 완료.');
    } catch (error) {
        console.error('temp_ingredient_master_data.json 쓰기 실패:', error);
    }
}

// 서버 시작 시 데이터 로드 및 디렉토리 확인
async function initializeMasterData() {
    await ensureDataDirectoryExists();
    initialRecipesMasterData = await readRecipesMasterData();
    tempIngredientMasterData = await readTempMasterData();
    console.log('모든 마스터 데이터 초기 로드 완료.');
}
initializeMasterData();


// 3. 헬퍼 함수 정의 (Helper Functions)
// Gemini API를 사용하여 식재료 상세 정보를 가져오는 함수 (재시도 로직 포함)
async function getIngredientDetailsFromAI(ingredientName, retries = 3, delay = 1000) {
    const geminiApiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${process.env.GEMINI_API_KEY}`;
    ...
    for (let i = 0; i < retries; i++) {
        try {
            if (!fetch) { // node-fetch가 아직 로드되지 않았을 경우 대기
                await new Promise(resolve => setTimeout(resolve, 100));
            }
            if (!fetch) { // 여전히 로드되지 않았다면 오류
                throw new Error("node-fetch 모듈이 로드되지 않았습니다.");
            }
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Gemini API 오류 응답 (식재료 상세) - 원본 텍스트:', errorText);
                if (response.status === 503 || errorText.includes('"code": 503') || errorText.includes('"status": "UNAVAILABLE"')) {
                    console.warn(`Gemini API 일시적 오류 (503 Service Unavailable) 발생. ${i + 1}번째 재시도 중...`);
            ...
            console.error(`Gemini를 통한 '${ingredientName}' 식재료 상세 정보 가져오기 실패:`, error);
            if (error.message.includes("Service Unavailable") && i < retries - 1) {
                console.warn(`Gemini API 일시적 오류로 재시도 중... (${i + 1}/${retries})`);
    ...
    console.error(`Gemini를 통한 '${ingredientName}' 식재료 상세 정보 가져오기 ${retries}회 시도 후 최종 실패.`);
    return null;
}
...
// --- 최종 데이터를 저장하는 API 엔드포인트 (name, date, quantity, storageMethod, isOpened 저장) ---
router.post('/save-products', auth, async (req, res) => {
    ...
    try {
        // temp_ingredient_master_data.json 최신 상태로 다시 로드 (동시 요청으로 인한 데이터 불일치 방지)
        tempIngredientMasterData = await readTempMasterData();

        const itemsToSaveToUser = []; // 사용자 데이터(User)에 저장될 식재료 목록
        const newIngredientDetailsForTempFile = []; // temp_ingredient_master_data.json에 추가될 새로운 식재료 마스터 정보
        ...
        for (const product of products) {
            ...
            // 1. 먼저 recipes.js에서 식재료 정보 조회
            masterInfo = initialRecipesMasterData.find(item => item.name === summarizedName);

            // 2. recipes.js에 없으면 temp_ingredient_master_data.json에서 식재료 정보 조회
            if (!masterInfo) {
                masterInfo = tempIngredientMasterData.find(item => item.name === summarizedName);
            }

            // 마스터 정보가 있으면 기본 보관 방법 사용
            if (masterInfo && masterInfo.defaultStoreMethod) {
                determinedStorageMethod = masterInfo.defaultStoreMethod;
            }

            // 사용자(User)에게 저장할 식재료 정보 구성
            itemsToSaveToUser.push({
                name: summarizedName,
                purchaseDate: formattedPurchaseDate,
                quantity: product.quantity || '있음',
                storageMethod: determinedStorageMethod,
                isOpened: product.isOpened !== undefined ? product.isOpened : false,
            });

            // 3. recipes.js에도 없고, temp_ingredient_master_data.json에도 없는 완전히 새로운 식재료인 경우 Gemini API 호출
            //    그리고 그 정보를 newIngredientDetailsForTempFile에 추가 (temp 파일에 저장하기 위함)
            if (!masterInfo) {
                console.log(`새로운 식재료 타입 발견: ${summarizedName}. Gemini로부터 상세 정보 가져오는 중...`);
                const details = await getIngredientDetailsFromAI(summarizedName);
                if (details) {
                    // 중복 추가 방지를 위해 이미 배열에 있는 내용은 추가하지 않음
                    // (recipes.js나 temp_ingredient_master_data.json에 이미 있는 경우도 고려)
                    if (!initialRecipesMasterData.some(item => item.name === details.name) &&
                        !tempIngredientMasterData.some(item => item.name === details.name) &&
                        !newIngredientDetailsForTempFile.some(item => item.name === details.name)) {
                        newIngredientDetailsForTempFile.push(details);
                    }
                }
            }
        }

        // 1. 사용자 식재료 저장 (User 모델에 추가)
        const savedCount = await ingredientService.addMultipleIngredientsToUser(userId, itemsToSaveToUser);
        console.log(`사용자 식재료 업데이트 완료. ${savedCount}개 항목 저장됨.`);

        // 2. 새로운 IngredientMaster 항목을 temp_ingredient_master_data.json 파일에 저장
        let newIngredientTypesAddedToTempFileCount = 0;
        if (newIngredientDetailsForTempFile.length > 0) {
            // 현재 temp 파일 데이터에 새로 가져온 마스터 데이터를 추가
            tempIngredientMasterData = tempIngredientMasterData.concat(newIngredientDetailsForTempFile);
            await writeTempMasterData(tempIngredientMasterData); // 파일에 쓰기
            newIngredientTypesAddedToTempFileCount = newIngredientDetailsForTempFile.length;
            console.log(`temp_ingredient_master_data.json 업데이트 완료. 새로운 식재료 타입: ${newIngredientTypesAddedToTempFileCount}개`);
        } else {
            console.log('temp_ingredient_master_data.json에 새로운 식재료 타입이 추가되지 않았습니다.');
        }

        res.json({
            message: '식재료 정보가 성공적으로 저장되었습니다.',
            savedCount: savedCount,
            newIngredientTypesAddedToTempFile: newIngredientTypesAddedToTempFileCount // temp 파일에 추가된 개수 응답
        });
